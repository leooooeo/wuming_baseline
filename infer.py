...
def _batch_to_model_input(
    ...
    for domain in seq_domains:
        ...
        if tf_key in device_batch:
            ...
            seq_ts_float_feats[domain] = torch.zeros(
                B, 5, L, dtype=torch.float32, device=device
            )
    ...
    if "timestamp" in device_batch:
        ...
        sample_timestamp = torch.zeros(B0, dtype=torch.int64, device=device)

    seq_event_ts: Dict[str, torch.Tensor] = {}
    for domain in seq_domains:
        ek = f"{domain}_event_ts"
        B, _, L = device_batch[domain].shape
        if ek in device_batch:
            seq_event_ts[domain] = device_batch[ek]
        else:
            seq_event_ts[domain] = torch.zeros(
                B, L, dtype=torch.int64, device=device
            )

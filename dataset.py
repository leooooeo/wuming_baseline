...
@njit(cache=True, fastmath=True)
def fill_seq_buffer_jit(out, lengths, offsets_tuple, values_tuple, max_len):
    B = out.shape[0]
    C = len(offsets_tuple)
...
PAIR_DENSE = [f"user_dense_feats_{fid}" for fid in [62, 63, 64, 65, 66]]
# fmt: on
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        for fid, dim in self._user_dense_cols:
            ...
            need_log1p = False
            if fid in PAIR_FEATURES and fid < 89:
                need_log1p = True
            self._user_dense_plan.append((ci, dim, offset, need_log1p))
            offset += dim
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        user_dense[:] = 0
        for ci, dim, offset, needlog1p in self._user_dense_plan:
            col = batch.column(ci)
        ...
        for domain in self.seq_domains:
            ...
            # ---- call JIT kernel ----
            fill_seq_buffer_jit(out, lengths, offsets_tuple, values_tuple, max_len)
            ...
            for c, vs in enumerate(vs_list):
                ...
                if vs > 0:
                    # # Hash vocab size
                    # if vs >= self.hash_threshold:
                    #     hash_ids_inplace(slice_c.reshape(-1), vs)
                    self._clip_vocab_size(slice_c, vs)
                else:
                    slice_c[:] = 0
            ...
            #   1: hours, 2: weeks
            #   3/4: sin/cos of hour-of-day (UTC-style, no timezone) from event_ts
            # Raw event Unix timestamps (aligned with sequence slots), for query stats.
            event_ts_buf = np.zeros((B, max_len), dtype=np.int64)
            time_bucket = self._buf_seq_tb[domain][:B]
            time_bucket[:] = 0
            ts_float_buf = np.zeros((B, 5, max_len), dtype=np.float32)
            if ts_ci is not None:
                ...
                row_lengths = np.diff(ts_offs)
                row_lengths = np.minimum(row_lengths, max_len)

                for i in range(B):
                    ul = row_lengths[i]
                    if ul > 0:
                        start = ts_offs[i]
                        ts_padded[i, :ul] = ts_vals[start : start + ul]
                ...
                ts_float_buf[:, 0, :] = np.log1p(d)
                ts_float_buf[:, 1, :] = d / 3600.0
                ts_float_buf[:, 2, :] = d / 604800.0
                h = (ts_padded // 3600) % 24
                ang = (2.0 * np.pi / 24.0) * h.astype(np.float64)
                ts_float_buf[:, 3, :] = np.where(valid, np.sin(ang).astype(np.float32), 0.0)
                ts_float_buf[:, 4, :] = np.where(valid, np.cos(ang).astype(np.float32), 0.0)

            result[f"{domain}_time_bucket"] = torch.from_numpy(time_bucket.copy())
            result[f"{domain}_ts_float_feats"] = torch.from_numpy(ts_float_buf.copy())
            result[f"{domain}_event_ts"] = torch.from_numpy(event_ts_buf.copy())

        return result

    def _register_pair(self, fid: int, kind: str, offset: int, dim: int, vs: int = 0):
        if fid not in self.pair_offsets:
            self.pair_offsets[fid] = {}

        self.pair_offsets[fid][kind] = (offset, offset + dim, vs)

    def _build_pair_offsets(self):
        # user_int
        for ci, dim, offset, vs in self._user_int_plan:
            col_name = self.idx_to_fid[ci]
            postfix = int(col_name.split("_")[-1])
            if postfix not in PAIR_FEATURES:
                continue
            self._register_pair(postfix, "user_int", offset, dim, vs)

        # user_dense
        for ci, dim, offset, _ in self._user_dense_plan:
            col_name = self.idx_to_fid[ci]
            postfix = int(col_name.split("_")[-1])
            if postfix not in PAIR_FEATURES:
                continue
            self._register_pair(postfix, "user_dense", offset, dim)
